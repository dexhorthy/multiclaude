...
export function loadConfig(): LauncherConfig {
  const repoName =
    process.env.MULTICLAUDE_REPO_NAME || path.basename(process.cwd());
  ...
  const config: LauncherConfig = {
    worktreeDir:
      process.env.MULTICLAUDE_WORKTREE_DIR || defaultConfig.worktreeDir,
    tmuxSession:
      process.env.MULTICLAUDE_TMUX_SESSION || defaultConfig.tmuxSession,
    repoName: repoName,
  ...
  const configPath =
    process.env.MULTICLAUDE_CONFIG || ".multiclaude/config.json";
  ...
  if (fs.existsSync(configPath)) {
    try {
      ...
      if (fileConfig.defaultBranch)
        config.defaultBranch = fileConfig.defaultBranch;
    } catch (error) {
      console.warn(
